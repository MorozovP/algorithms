...
def partition(n: List[Participant], low: int, high: int) -> int:
    """
    Вспомогательная функция быстрой сортировки. Сравнивает элементы массива с
    опорным элементом и перемещает элементы меньшие опорного в начало массива,
    элементы больше опорного - в конец. Возвращает индекс опорного элемента.
    """
    i: int = low - 1
